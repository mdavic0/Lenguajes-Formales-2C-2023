...
(add-rule 
    (make-rule "Explode by special characters"
        (let ((fn (getf arg :fn))
            ...
            (list :fn (string-explode-sequentially fn "(" ")" "/" "->" "<>" ">" "<" "[" "]" "~" "+" "-" "*" "%" "@" "1R" "2R" "3R" "4R" "5R" "6R" "7R" "8R" "9R")
...
; selector-right-1
;;------------------------------
(defun selector-right-1 ()
    (lambda (arg)
        (if (<= 1 (length arg))
            (nth 0 (reverse arg))
            (error (format nil "Wrong argument: ~a. Expected length: ~a~%" (desymbolize arg) 1)))))

;;------------------------------
; selector-right-2
;;------------------------------
(defun selector-right-2 ()
    (lambda (arg)
        (if (<= 2 (length arg))
            (nth 1 (reverse arg))
            (error (format nil "Wrong argument: ~a. Expected length: ~a~%" (desymbolize arg) 2)))))

;;------------------------------
; selector-right-3
;;------------------------------
(defun selector-right-3 ()
    (lambda (arg)
        (if (<= 3 (length arg))
            (nth 2 (reverse arg))
            (error (format nil "Wrong argument: ~a. Expected length: ~a~%" (desymbolize arg) 3)))))

;;------------------------------
; selector-right-4
;;------------------------------
(defun selector-right-4 ()
    (lambda (arg)
        (if (<= 4 (length arg))
            (nth 3 (reverse arg))
            (error (format nil "Wrong argument: ~a. Expected length: ~a~%" (desymbolize arg) 4)))))

;;------------------------------
; selector-right-5
;;------------------------------
(defun selector-right-5 ()
    (lambda (arg)
        (if (<= 5 (length arg))
            (nth 4 (reverse arg))
            (error (format nil "Wrong argument: ~a. Expected length: ~a~%" (desymbolize arg) 5)))))

;;------------------------------
; selector-right-6
;;------------------------------
(defun selector-right-6 ()
    (lambda (arg)
        (if (<= 6 (length arg))
            (nth 5 (reverse arg))
            (error (format nil "Wrong argument: ~a. Expected length: ~a~%" (desymbolize arg) 6)))))

;;------------------------------
; selector-right-7
;;------------------------------
(defun selector-right-7 ()
    (lambda (arg)
        (if (<= 7 (length arg))
            (nth 6 (reverse arg))
            (error (format nil "Wrong argument: ~a. Expected length: ~a~%" (desymbolize arg) 7)))))

;;------------------------------
; selector-right-8
;;------------------------------
(defun selector-right-8 ()
    (lambda (arg)
        (if (<= 8 (length arg))
            (nth 7 (reverse arg))
            (error (format nil "Wrong argument: ~a. Expected length: ~a~%" (desymbolize arg) 8)))))

;;------------------------------
; selector-right-9
;;------------------------------
(defun selector-right-9 ()
    (lambda (arg)
        (if (<= 9 (length arg))
            (nth 8 (reverse arg))
            (error (format nil "Wrong argument: ~a. Expected length: ~a~%" (desymbolize arg) 9)))))
...
(defun init-functions ()
    ...
    (add-function (make-function "1r" #'selector-right-1))
    (add-function (make-function "2r" #'selector-right-2))
    (add-function (make-function "3r" #'selector-right-3))
    (add-function (make-function "4r" #'selector-right-4))
    (add-function (make-function "5r" #'selector-right-5))
    (add-function (make-function "6r" #'selector-right-6))
    (add-function (make-function "7r" #'selector-right-7))
    (add-function (make-function "8r" #'selector-right-8))
    (add-function (make-function "9r" #'selector-right-9))
